...
let asteroidSpeed = 1.0; // Уменьшена начальная скорость
let asteroidSize = 56;
...
function preloadAssets(callback) {
    let loadedAssets = 0;
    const assetsToLoad = 5; // Reduced from 6 to 5 since we removed logo
...
function initGame() {
    preloadAssets(() => {
        setupCanvas();
        setupEventListeners();
        document.getElementById('gameContainer').style.border = '4px solid #00a2ff'; 
        document.getElementById('gameContainer').style.boxShadow = '0 0 20px rgba(0, 162, 255, 0.5)';
...
function keyDownHandler(e) {
    ...
    if (keys.hasOwnProperty(e.key)) {
        keys[e.key] = true;
        console.log('Keys state:', JSON.stringify(keys));
...
function resetGame() {
    ...
    asteroidSpeed = 1.0; // Уменьшена начальная скорость
    asteroidSize = 56;
    bullets = [];
    asteroids = [];
    lastAsteroidTime = 0;
...
function updateAsteroids(timestamp) {
    if (timestamp - lastAsteroidTime > asteroidInterval / difficulty) {
        ...
        // Ограничиваем максимальный счет для расчета скорости 250 очками
        const effectiveScore = Math.min(score, 250);
        // Устанавливаем фиксированную скорость после 250 очков
        asteroidSpeed = Math.min(2, 1.0 + (effectiveScore / 125) * 0.5);
        // Увеличиваем минимальный интервал между астероидами
        asteroidInterval = Math.max(400, 2000 - (Math.min(score, 250) * 2));
        asteroidSize = Math.min(80, 56 + (effectiveScore / 10));
    }
